...
- **流式输出与等待提示** — 请求后立即显示 `Thinking...`，首个事件到达后逐步渲染助手文本
- **会话差异与撤销** — `/diff` 查看本次会话修改，`/undo` 按步骤回滚最近一次文件变更
- **交互式审批** — 写入或执行时支持 `y/n/a/d`，危险操作始终硬拒绝
- **工具耗时显示** — 每次工具执行后展示实际耗时
...
一次性 `chat` 与交互式 REPL 共用不依赖 UI 的 `AgentLoopCore`，统一完成响应归一化和工具执行。REPL 在共享核心外增加首事件前的 `Thinking...` 状态、流式渲染、交互审批、持久上下文、`/diff` 和 `/undo`。等待提示仅表示请求仍在处理，不展示或伪造模型隐藏推理内容。

## 开发状态

- 当前包版本：**v0.2.0**
- Phase 2A 已完成：流式输出、首事件等待提示、TTFT/总耗时指标、工具耗时、交互式审批、`/diff`、`/undo`
- 项目主测试集 **492** 项通过（收集时排除 fixture 与 E2E 数据项目）
